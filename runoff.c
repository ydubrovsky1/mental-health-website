...
int find_min(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false)
        {
            ...
            {
                //update the lowest vote count as you find candidates with lower votes
                minvotes = candidates[i].votes;
            }
        }
    }
    //return the minimum vote-count of any candidate still in the race
    return minvotes;
}
    ...
    for (int i = 0; i < candidate_count; i++)
        {
            if (candidates[i].eliminated == false)
            {
                ...
                {
                    //if any candidate still in the election does not have the minimum vote number, it is not a tie
                    return false;
                }
            } 
        }
    //if all of the candidates had the minimum vote number, the function returns true
    return true;
}
